...
test_config = [{'lanes': [LaneSpec(6, [-2, -2])] * 2 + [LaneSpec(6, [-5, -5])] * 2 +
               ...
               {'lanes': [LaneSpec(6, [-2, -2])] * 2 + [LaneSpec(4, [-3, -3])] * 2 + [LaneSpec(6, [-4, -4])] * 2 + [
                   LaneSpec(4, [-4, -4])] * 2 + [LaneSpec(6, [-2, -2])] * 2 + [LaneSpec(4, [-3, -3])] * 2, 'width': 35,
                'seed': 101}]
...
class GeneratePDDL_Stationary:
    ...
    def generateTime(self):
        ...
        '''
        max = self.state.agent.position.x - self.state.finish_position.x + 1
        self.time_list = []
        for w in range(max):
    ...
    def generateInitString(self):
        ...
        init_string = ""
        width = self.width
        cars_copy = self.state.cars.copy()
        for car in cars_copy:
            speed = 0 - car.speed_range[0]
            # print("car id:", car.id, "x: ", car.position.x, "y:", car.position.y, "speed range :", car.speed_range)
            init_string += "(blocked pt" + str(car.position.x) + "pt" + str(car.position.y) + " t0)"
            car.position.x = car.position.x - speed + width if car.position.x - speed < 0 else car.position.x - speed
        for t in range(1, len(self.time_list)):
            for car in cars_copy:
                speed = 0 - car.speed_range[0]
                init_string += "(blocked pt" + str(car.position.x) + "pt" + str(car.position.y) + " t" + str(t) + ")"
                for i in range(1, speed):
                    init_string += "(blocked pt" + str(
                        car.position.x + i if car.position.x + i < width else car.position.x + i - width) + "pt" + str(
                        car.position.y) + " t" + str(t) + ")"
                car.position.x = car.position.x - speed + width if car.position.x - speed < 0 else car.position.x - speed
        init_string += "(at pt" + str(self.state.agent.position.x) + "pt" + str(
            self.state.agent.position.y) + " agent1 t0)"
        init_string += "(blocked pt" + str(self.state.agent.position.x) + "pt" + str(
            self.state.agent.position.y) + "  t0)"
        '''grid cells relations'''
        for i in range(self.num_lanes):
            for j in range(1, self.width):
                init_string += "(up_next pt" + str(j) + "pt" + str(i) + " " + "pt" + str(j - 1) + "pt" + str(
                    0 if i == 0 else i - 1) + ")"
                init_string += "(down_next pt" + str(j) + "pt" + str(i) + " " + "pt" + str(j - 1) + "pt" + str(
                    self.num_lanes - 1 if i == self.num_lanes - 1 else i + 1) + ")"
                init_string += "(forward_next pt" + str(j) + "pt" + str(i) + " " + "pt" + str(j - 1) + "pt" + str(
                    i) + ")"
        '''time relations'''
        for t in range(1, len(self.time_list)):
            init_string += "(next_time " + self.time_list[t - 1] + " " + self.time_list[t] + ")"
        return init_string
    ...
    def generateGoalString(self):
        ...
        for t in self.time_list:
            goal_string += "(at pt" + str(self.state.finish_position.x) + "pt" + str(
                self.state.finish_position.y) + " agent1 " + t + ")"
        goal_string += ")"
        return goal_string
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP",
                  parameters=(
                      ("agent", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (next_time ?t1 ?t2) (at ?pt1 ?agent ?t1) (up_next ?pt1 ?pt2) (not (blocked ?pt2 ?t2)))",
                  effect_string="(and (at ?pt2 ?agent ?t2) (blocked ?pt2 ?t2) (not (blocked ?pt1 ?t2)))")
    gen.addAction(name="DOWN",
                  parameters=(
                      ("agent", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (next_time ?t1 ?t2) (at ?pt1 ?agent ?t1) (down_next ?pt1 ?pt2) (not (blocked ?pt2 ?t2)))",
                  effect_string="(and (at ?pt2 ?agent ?t2) (blocked ?pt2 ?t2) (not (blocked ?pt1 ?t2)))")
    gen.addAction(name="FORWARD1",
                  parameters=(
                      ("agent", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (next_time ?t1 ?t2) (at ?pt1 ?agent ?t1) (forward_next ?pt1 ?pt2) (not (blocked ?pt2 ?t2)))",
                  effect_string="(and (at ?pt2 ?agent ?t2) (blocked ?pt2 ?t2) (not (blocked ?pt1 ?t2)))")
    gen.addAction(name="FORWARD2",
                  parameters=(
                      ("agent", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell"), ("pt3", "gridcell"), ("t1", "time"),
                      ("t2", "time")),
                  precondition_string="(and (next_time ?t1 ?t2) (at ?pt1 ?agent ?t1) (forward_next ?pt1 ?pt2) (forward_next ?pt2 ?pt3) (not (blocked ?pt3 ?t2)))",
                  effect_string="(and (at ?pt3 ?agent ?t2) (blocked ?pt3 ?t2) (not (blocked ?pt1 ?t2)))")
    gen.addAction(name="FORWARD3",
                  parameters=(
                      ("agent", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell"), ("pt3", "gridcell"),
                      ("pt4", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (next_time ?t1 ?t2) (at ?pt1 ?agent ?t1) (forward_next ?pt1 ?pt2) (forward_next ?pt2 ?pt3) (forward_next ?pt3 ?pt4) (not (blocked ?pt4 ?t2)))",
                  effect_string="(and (at ?pt4 ?agent ?t2) (blocked ?pt4 ?t2) (not (blocked ?pt1 ?t2)))")

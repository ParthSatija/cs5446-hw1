...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        for car in self.state.cars:
            init_string += "(blocked pt" + str(car.position.x) + "pt" + str(car.position.y) + ")"
        init_string += "(at pt" + str(self.state.agent.position.x) + "pt" + str(self.state.agent.position.y) + " agent1)"
        init_string += "(blocked pt" + str(self.state.agent.position.x) + "pt" + str(self.state.agent.position.y) + ")"
        '''up_next'''
        for i in range(self.num_lanes):
            for j in range(1, self.width):
                init_string += "(up_next pt" + str(j) + "pt" + str(i) + " " + "pt" + str(j - 1) + "pt" + str(0 if i == 0 else i -1) + ")"
                init_string += "(down_next pt" + str(j) + "pt" + str(i) + " " + "pt" + str(j - 1) + "pt" + str(self.num_lanes - 1 if i == self.num_lanes - 1 else i + 1) + ")"
                init_string += "(forward_next pt" + str(j) + "pt" + str(i) + " " + "pt" + str(j - 1) + "pt" + str(i) + ")"
        return init_string
    ...
    def generateGoalString(self) :
        ...
        '''
        goal_string = "(at pt" + str(self.state.finish_position.x) + "pt" +str( self.state.finish_position.y) + " agent1)"
        return goal_string
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP",
                  ...
                  effect_string="(and (at ?pt2 ?agent) (up_next ?pt1 ?pt2) (blocked ?pt2) (not (blocked ?pt1)))")
    gen.addAction(name="DOWN",
                  parameters=(("agent", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell")),
                  precondition_string="(and (at ?pt1 ?agent) (down_next ?pt1 ?pt2) (not (blocked ?pt2)))",
                  effect_string="(and (at ?pt2 ?agent) (down_next ?pt1 ?pt2) (blocked ?pt2) (not (blocked ?pt1)))")
    gen.addAction(name="FORWARD",
                  parameters=(("agent", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell")),
                  precondition_string="(and (at ?pt1 ?agent) (forward_next ?pt1 ?pt2) (not (blocked ?pt2)))",
                  effect_string="(and (at ?pt2 ?agent) (forward_next ?pt1 ?pt2) (blocked ?pt2) (not (blocked ?pt1)))")
